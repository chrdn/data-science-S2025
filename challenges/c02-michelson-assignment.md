...
Our MeanVelocity has an additional decimal point. It seems like our data
set is also ~2 m/s off from the one Michelson created.

\- Why might your table differ from Michelson’s? - (Your response here)

This might be the limit of Michelson’s tools, as it looks like there is
a consistent bias. Additionally, he might have decided to round, as his
table ends in factors of ten.
...
His estimate is less than the true error
...
The real speed of light is ~100 km/s lower than his lower bound.
...
- They seem to have similar shapes. The real data spikes with the
  simulated data and drops with the simulated data.

Differences

- Real data tends to “spike” a lot more steeply. Even the mean goes over
  the error range.

- The real data collected tends to be “grouped” more. The simulated data
  has many points on both sides of the error bars that get “even out” to
  a mean that is contained within the error bars.
