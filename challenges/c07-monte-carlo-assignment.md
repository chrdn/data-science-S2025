...
df_q1 <- tibble(x = runif(n,0,1), y = runif(n,0,1)) # Generate the data
```
...
assertthat::assert_that(
  tibble(x = .99, y = .9) %>% # Pick a point *outside* the circle
    mutate(stat = stat(x,y)) %>% 
    pull(stat) %>% 
    .[[1]] == 0,
  # ???, # Write the correct value of stat() here
...
- What is a range of plausible values, based on the sampling
  ...
  - It looks like most of the values are between 3.135 to 3.143
  - The true minimum of the sample is ~3.130 while the true maximum is ~
    3.147
...
``` r
mean_pi <- mean(df_q4$pi_est)
sd_pi <- sd(df_q4$pi_est)
...
- How closely do your bootstrap CI and CLT CI agree?
  - My bootstrap CI and CLT CI agree to the thousandths place.
- Comment on the width of your CI(s). Would your estimate of $\pi$ be
  ...
  orbit)?
  - For the art project, we only need a rough estimation of the area as
    it would be impossible to buy exactly enough paint anyways.
    Therefore, using this estimate of $\pi$ would be good enough for
    this purpose.
  - As a precise estimation, the lower bound is 3 thousandths off from
    the true value of $\pi$ and 6 thousandths off from the true value of
    $\pi$. Although this seems relatively small, for the purposes of
    launching a rocket, it may be better to be accurate to at least
    $3.141$ if not further.
- What would be a *valid* way to make your CI more narrow?
  - Increasing the sample size should make the CI more narrow. However,
    there is a trade-off with calculation time.
